...
def IAChoice(allu) :
    ...
    # J'ai utilisé la variable stock, qui ne dépend pas directement de allu
    #(je pouvais pas l'initialiser à [0,0,0,0], sinon le programme ne rentrait pas du tout dans le while)
    stock = allu.copy()
    while not paire(stock) != Mod:
...
def Game ():
    ...
    allu=[1,3,5,7]
    win= False
    choiceMod = int(input("0 : celui qui retire la derniere allumette a gagné \n 1: celui qui laisse la derniere allumette a gagné"))
    while choiceMod != 1 and choiceMod != 0:
        choiceMod = int(input(" Rechoissir \n 0 : celui qui retire la derniere allumette a gagné \n 1: celui qui laisse la derniere allumette a gagné"))
        if choiceMod == 0 :
            Mod = True
        else :
            Mod = False
    while allu!=[0,0,0,0]:
        ...
        while retireJ > allu[ligneJ-1]:
            retireJ = int(input("Vous avez choisis trop d'allumettes. veuillez ne pas tricher !!"))
